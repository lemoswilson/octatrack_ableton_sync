...
class octatrack_ableton_sync(ControlSurface):
	def __init__(self, c_instance):
		...
		with self.component_guard():
			...
			self.add_scenes_listeners(self.scenes_listener_handler)
	...
	def find_pcc_index(self): 
		return find_index_of(self.pcc_key)

	# Get the PCA index
	def find_pca_index(self):
		return find_index_of(self.pca_key)

	# Get the IPCS index
	def find_ipcs_index(self):
		return find_index_of(self.ipcs_key)
	...
	def reset_notes_listener(self, clip):
		clip.remove_notes_listener(notes_listener_handler(clip))
		clip.add_notes_listener(notes_listener_handler(clip))
	...
	def notes_listener_handler(self, clip):
		def inner(): 
			...
			pca_clip_slot = song.tracks[self.pca_index].clip_slots[clip_index]
			pca_notes = self.make_pca_clip_notes_from_pcc_clip(clip)


			# Get the IPCS clip slot and make its notes
			ipcs_clip_slot = song.tracks[self.ipcs_index].clip_slots[clip_index]
			...
			if pca_clip_slot.has_clip:
				pca_clip_slot.clip.delete_notes()
			...
			if ipcs_clip_slot.has_clip:
				ipcs_clip_slot.clip.delete_notes()
	...
	def match_empty_clip_slots(self):
		for index, clip_slot in enumerate(self.song().track[self.pcc_key].clip_slots):
			if not clip_slot.has_clip:
				self.clear_pca_ipcs_clip_slot(self, index)
	...
	def clear_pca_ipcs_clip_slot(self, index):
		...
		try: 
			song.tracks[self.pca_key].clip_slots[index].delete_clip()
		except: 
			pass
	...
	def get_clip_index(self, clip, track_index):
		for index, clip_slot in enumerate(self.song().tracks[track_index].clip_slots)
			...
			else if clip_slot.clip.name == clip.name: return index
	...
	def make_pca_clip_notes_from_pcc_clip(self, pcc_clip):
		...
		return tuple([self.make_pca_note_from_pcc_note(note, pcc_clip) for note in notes])
	...
	def make_ipcs_clip_notes_from_pcc_clip(self, pcc_clip):
		...
		return tuple([self.make_ipcs_note_from_pcc_note(note, pcc_clip) for note in notes])
